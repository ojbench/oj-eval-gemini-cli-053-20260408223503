  ...
  */
  RegexChecker(const std::string &regex) {
    ...
    for (const std::string &part : parts) {
      std::vector<NFA> concat_nfas;
      for (size_t i = 0; i < part.length(); ++i) {
        char c = part[i];
        if (i + 1 < part.length() && (part[i+1] == '+' || part[i+1] == '*' || part[i+1] == '?')) {
          if (part[i+1] == '+') {
            concat_nfas.push_back(MakePlus(c));
          } else if (part[i+1] == '*') {
            concat_nfas.push_back(MakeStar(c));
          } else if (part[i+1] == '?') {
            concat_nfas.push_back(MakeQuestion(c));
          }
          ++i; // skip the operator
        } else {
          concat_nfas.push_back(MakeSimple(c));
        }
      }
      
      NFA current_concat = concat_nfas[0];
      for (size_t i = 1; i < concat_nfas.size(); ++i) {
        current_concat = Concatenate(current_concat, concat_nfas[i]);
      }
      union_nfas.push_back(current_concat);
    }
...
NFA MakePlus(const char &character) {
  ...
  nfa.start = 0;
  nfa.ends.insert(1);
  nfa.transitions.push_back(std::vector<Transition>()); // state 0
  nfa.transitions.push_back(std::vector<Transition>()); // state 1
  TransitionType type = (character == 'a') ? TransitionType::a : TransitionType::b;
  nfa.transitions[0].push_back({type, 1});
...
NFA Union(const NFA &nfa1, const NFA &nfa2) {
  NFA nfa;
  nfa.start = 0;
  nfa.transitions.push_back(std::vector<Transition>()); // new start state
